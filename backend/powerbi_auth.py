...
CLIENT_SECRET = os.getenv("CLIENT_SECRET")
REDIRECT_URI = os.getenv("REDIRECT_URI")
SCOPES = os.getenv("POWER_BI_SCOPES", "https://analysis.windows.net/powerbi/api/.default offline_access")

if not (TENANT_ID and CLIENT_ID and CLIENT_SECRET and REDIRECT_URI):
    logger.warning("⚠️ Missing Power BI environment variables. Please check your .env file.")
...
# --- SIMPLE TOKEN STORE (for demo only) ---
TOKEN_STORE = {}
...
# -----------------------------
# STEP 1: LOGIN (redirect to Microsoft)
# -----------------------------
@router.get("/api/powerbi/login")
async def powerbi_login():
    """
    Redirects user to Azure AD sign-in to give consent for Power BI API.
    """
    url = build_authorize_url()
    return RedirectResponse(url)
...
async def powerbi_callback(request: Request):
    """
    Handles callback from Azure AD and exchanges 'code' for tokens.
    Returns a token_key for future Power BI API calls.
    """
    params = dict(request.query_params)

    if "error" in params:
        raise HTTPException(status_code=400, detail=f"Auth error: {params.get('error_description', '')}")

    code = params.get("code")
    if not code:
        raise HTTPException(status_code=400, detail="Authorization code missing from callback URL.")
    ...
    # Generate unique key for user session
    user_key = f"user_{int(time.time())}"
    ...
    logger.info("✅ Power BI token stored for %s", user_key)

    return JSONResponse({
        "message": "Power BI authentication successful.",
        "token_key": user_key,
        "expires_in": expires_in
    })
...
def get_valid_token_for(token_key: str):
    record = TOKEN_STORE.get(token_key)
    if not record:
        raise HTTPException(status_code=401, detail="No token found. Please authenticate again.")

    # Refresh if expired
    if time.time() > record["expires_at"] - 60:
        refresh_token = record.get("refresh_token")
        if not refresh_token:
            raise HTTPException(status_code=401, detail="No refresh token available; please re-login.")
        ...
        if r.status_code != 200:
            logger.error("🔁 Refresh token failed: %s", r.text)
            TOKEN_STORE.pop(token_key, None)
            raise HTTPException(status_code=401, detail="Token refresh failed; please re-authenticate.")
...
async def list_dashboards(token_key: str):
    """
    Fetches all dashboards from user's Power BI account using token_key.
    Example:
    GET /api/powerbi/dashboards?token_key=user_1730978432
    """
    access_token = get_valid_token_for(token_key)
